...
impl Session {
  ...
  pub fn get_valid_session(
    ...
    Self::new_from_store(&session).map_or_else(
      || Err(ApiError::new(401, "No session".to_string())),
      |finch_session| match finch_session.is_valid(col) {
  ...
  fn new_from_store(session: &actix_session::Session) -> Option<Session> {
  ...
  fn is_valid(&self, col: Collection) -> bool {
    col
      .find_one(
